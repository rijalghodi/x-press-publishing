...
class Landing extends Component {
  ...
  renderArtists() {
    return this.state.artists.map((artist) => {
      return (
        <Link to={`/artists/${artist.id}`} className="item" key={artist.id}>
          <h3>{artist.name}</h3>
  ...
  render() {
    return (
      <div className="Landing">
        ...
        <div className="series item-list">{this.renderSeries()}</div>
        <Link to="/series/new" className="button">
          ADD
        </Link>
        <h2>ARTISTS</h2>
        <div className="artist item-list">{this.renderArtists()}</div>
        <Link to="/artists/new" className="button">
          ADD
        </Link>
      </div>
    );
