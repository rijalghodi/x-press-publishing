...
class Series extends Component {
  ...
  cancelIssueEdit(issueIndex) {
    ...
    if (!issue.id) {
      ...
      issues[issueIndex] = JSON.parse(
        JSON.stringify(this.state.savedIssues[issueIndex])
      );
      this.setState({
        issues: issues,
      });
  ...
  renderSeriesButtons() {
    ...
    if (this.seriesHasChanges() && this.seriesHasAllRequiredFields()) {
      saveButton = (
        <a className={"button"} onClick={this.saveSeries}>
          Save
        </a>
      );
    } else {
    ...
    if (this.seriesHasChanges()) {
      cancelButton = (
        <a className={"button"} onClick={this.cancelSeriesEdit}>
          Cancel
        </a>
      );
    } else {
    ...
    if (!this.state.issues.length) {
      deleteButton = (
        <a className="button delete" onClick={this.deleteSeries}>
          Delete
        </a>
      );
    } else {
  ...
  renderIssueButtons(issue, issueIndex) {
    ...
    if (
      this.issueHasChanges(issue, issueIndex) &&
      this.issueHasAllRequiredFields(issue)
    ) {
      saveButton = (
        <a className={"button"} onClick={this.saveIssue.bind(this, issueIndex)}>
          Save
        </a>
      );
    } else {
    ...
    if (this.issueHasChanges(issue, issueIndex)) {
      cancelButton = (
        <a
          className={"button"}
          onClick={this.cancelIssueEdit.bind(this, issueIndex)}
        >
          Cancel
        </a>
      );
    ...
    deleteButton = (
      <a
        ...
        onClick={this.deleteIssue.bind(this, issue, issueIndex)}
      >
        Delete
      </a>
    );
  ...
  renderIssues() {
      ...
      return (
        <div className="issue" key={issue.id}>
          <h3>
            Name:{" "}
            <input
              onChange={(e) => this.updateIssueName(e, issueIndex)}
              value={issue.name}
            />
          </h3>
          <p>
            ...
            <input
              type="number"
              onChange={(e) => this.updateIssueNumber(e, issueIndex)}
          ...
          <p>
            Date:{" "}
            <input
              onChange={(e) => this.updateIssueDate(e, issueIndex)}
              value={issue.publicationDate}
            />
          </p>
          <p>
            Artist:{" "}
            <select
              onChange={(e) => this.updateIssueArtist(e, issueIndex)}
              value={issue.artistId}
            >
    ...
    return (
      <div className="issue-container">
        ...
        <a className="button" onClick={this.addIssue}>
          Add Issue
        </a>
      </div>
    );
    ...
    return (
      <div className="Series">
        <div className="description">
          <input
            onChange={this.updateSeriesName}
            value={series.name}
            placeholder="Series Title"
          />
          <h3>Description</h3>
          <textarea
            onChange={this.updateSeriesDescription}
